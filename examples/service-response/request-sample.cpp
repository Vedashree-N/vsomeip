...
public:
    ...
    bool init() {
        ...
        const vsomeip::byte_t its_payload_data[]
                    = { 0x4, 0x3, 0x7};
    ...
    void on_message(const std::shared_ptr< vsomeip::message > &_response) {
        ...
        its_message << "Received a response from Service ["
        ...
        its_message << "(" << std::dec << resp_payload->get_length() << ") "
		    << std::hex << std::setw(2);
        for (uint32_t i = 0; i < resp_payload->get_length(); ++i){
            its_message << std::setw(2) << (int) resp_payload->get_data()[i] << " ";
            }
