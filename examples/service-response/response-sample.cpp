...
public:
	...
	std::stringstream its_message;
        its_message << "Received a Request from client with  ["
                << std::setfill('0') << std::hex
                << std::setw(4) << _request->get_service() << "."
                << std::setw(4) << _request->get_instance() << "."
                << std::setw(4) << _request->get_method() << "] to Client/Session ["
                << std::setw(4) << _request->get_client() << "/"
                << std::setw(4) << _request->get_session()
                << "] Client payload is = ";
        std::shared_ptr<vsomeip::payload> req_payload =
                _request->get_payload();
        its_message << "(" << std::dec << req_payload->get_length() << ") "
		    << std::hex << std::setw(2);
        for (uint32_t i = 0; i < req_payload->get_length(); ++i){
            its_message << std::setw(2) << (int) req_payload->get_data()[i] << " ";
            payload_data[i]=(int) req_payload->get_data()[i];
            chk_payload=true;
        }
        ...
        for (std::size_t i = payload_data[1]; i <payload_data[2]+payload_data[1]; ++i)
        ...
        if (use_static_routing_) {
            ...
            while (running_) {
                if (is_offer)
                    ...
                    stop_offer();
		if(chk_payload==false)
		{
		  payload_data[0]=3;
		}
                for (int i = 0; i < payload_data[0] && running_; i++)
